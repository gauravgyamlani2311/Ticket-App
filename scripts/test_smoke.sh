...
curl -s --head URL="http://54.226.233.59:8081/index.html" | head -n 1 | grep "200" > /dev/null
WEB_RES=$?

# Test 2: PHP/DB Connectivity (via the info.php page)
curl -s http://54.226.233.59:8081/info.php | grep "Connected to Database" > /dev/null
DB_RES=$?
